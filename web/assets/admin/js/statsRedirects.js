...
function loadOffers(page) {
    ...
    xhr.open("GET", "/admin/stat-redirects-async?page=" + page + "&perPage=" + perPage, true);
...
function renderOffers(offers) {
    ...
    html += "<thead><tr><th scope='col'>ID</th><th scope='col'>Название офера</th><th scope='col' >Редирект</th><th scope='col'>URL</th><th scope='col'>Тематика сайта</th><th scope='col'>веб-мастер</th><th scope='col'>ip</th><th scope='col'>Дата и время</th></tr></thead>";
    html += "<tbody>";
    for (let i = 0; i < offers.length; i++) {
        html += "<tr>";
        html += "<td>" + offers[i].id + "</td>";
        html += "<td>" + offers[i].advertiser_name + "</td>";
        html += "<td>" + (offers[i].redirected === '1' ? '<span class="badge badge-success">да</span>' : '<span class="badge badge-danger">нет</span>') + "</td>";
        ...
        html += "<td>" + (offers[i].advertiser_theme === null ? '<i>(не указано)</i>' : offers[i].advertiser_theme) + "</td>";
        html += "<td>" + offers[i].username + "</td>";
        html += "<td>" + offers[i].ip + "</td>";
        html += "<td>" + offers[i].redirect_date + "</td>";
        html += "</tr>";
    }
    html += "</tbody></table>";
    return html;
