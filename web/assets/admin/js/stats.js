document.getElementById('statsForm').addEventListener('submit', function (event) {
    ...
    xhr.open('POST', '/admin/get-stats', true);
    xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            document.getElementById("totalIncome").innerHTML = "всего заработано <b>" + totalIncome.toFixed(2) + " ₽</b>";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function loadOffers() {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/admin/get-offers-list", true);
        xhr.onload = function () {
            if (xhr.status >= 200 && xhr.status < 300) {
