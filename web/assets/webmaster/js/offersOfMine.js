...
function loadOffers(page) {
    ...
    xhr.open("POST", "/webmaster/get-list-subscribed?page=" + page + "&perPage=" + perPage, true);
...
function renderOffers(offers) {
    ...
    html += "<thead><tr><th scope='col'>ID</th><th scope='col'>Название</th><th scope='col'>URL для размещения</th><th scope='col'>Стоимость клика,  ₽</th><th scope='col'>Дата и время</th><th scope='col'>Отписатся</th></tr></thead>";
    html += "<tbody>";
    for (let i = 0; i < offers.length; i++) {
        html += "<tr>";
        html += "<td>" + offers[i].id + "</td>";
        html += "<td><b>" + offers[i].name + "</b></td>";
        html += "<td><pre><small>" + offers[i].url + "</small></pre></td>"; // Вывод URL оффера
        html += "<td><b>" + offers[i].discounted_cost_per_click + " ₽</b></td>";
        html += "<td>" + offers[i].timestamp + "</td>";
        html += "<td><a class='subscribe' href='/webmaster/unsubscribe?id=" + offers[i].id + "'>X</a></td>";
        html += "</tr>";
    }
    html += "</tbody></table>";
    return html;
